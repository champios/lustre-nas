			       ...
			       loff_t *pos)
{
	...
	loff_t offset = *pos & NRS_TBF_STATS_OFF_MASK;
	loff_t idx = 0;
	...
	list_for_each_entry(client, &head->th_cli_list, tc_head_linkage) {
		if (offset == idx) {
			found = client;
			break;
		}
	...
	/* Find no client in the given position, try next one */
	if (found == NULL)
...
static struct nrs_tbf_client *
nrs_tbf_stats_get_client(struct ptlrpc_service *svc, loff_t *pos)
{
	struct ptlrpc_service_part *svcpt;
	struct nrs_tbf_client *cli = NULL;
	bool has_hp = nrs_svc_has_hp(svc);
	loff_t cpt;

	cpt = *pos >> NRS_TBF_STATS_CPT_SHIFT;
	while (cpt < svc->srv_ncpts) {
		bool hp = false;

		svcpt = svc->srv_parts[cpt];
		if (has_hp)
			hp = *pos & NRS_TBF_STATS_CPT_HP;
		cli = nrs_tbf_stats_get_client_svcpt(svcpt, hp, pos);
		if (cli)
			break;

		if (!hp && has_hp) {
			*pos = (*pos & ~NRS_TBF_STATS_OFF_MASK) |
			       NRS_TBF_STATS_CPT_HP;
		} else {
			cpt++;
			*pos = cpt << NRS_TBF_STATS_CPT_SHIFT;
		}
	}
...
static void nrs_tbf_stats_seq_stop(struct seq_file *p, void *v)
{
	 ...
	 * nrs_tbf_stats_get_client() perviously. Release it now.
	 */
	read_unlock(&head->th_cli_lock);
}
...
static void *nrs_tbf_stats_seq_next(struct seq_file *p, void *v, loff_t *pos)
{
	...
	__u32 offset = *pos & NRS_TBF_STATS_OFF_MASK;
	struct list_head *next;

	next = cli->tc_head_linkage.next;
	if (offset >= NRS_TBF_STATS_OFF_MAX || next == &head->th_cli_list) {
		/*
		 * The th_cli_lock should have been got when calling
		 * nrs_tbf_stats_get_client() previously. Release the lock
		 * now.
		 */
		read_unlock(&head->th_cli_lock);

		if ((*pos & NRS_TBF_STATS_CPT_HP) && nrs_svc_has_hp(svc)) {
			/* Try HP svc part. */
			*pos = (*pos & ~NRS_TBF_STATS_OFF_MASK) |
			       NRS_TBF_STATS_CPT_HP;
		} else {
			...
			/* Try next svc CPT. */
			cpt = *pos >> NRS_TBF_STATS_CPT_SHIFT;
			*pos = (cpt + 1) << NRS_TBF_STATS_CPT_SHIFT;
		}

		cli = nrs_tbf_stats_get_client(svc, pos);
	} else {
		++*pos;
