			      ...
			      struct tbf_id *id)
{
	...
	if (body != NULL) {
		if (id->ti_type & NRS_TBF_FLAG_UID)
			id->ti_uid = body->oa.o_uid;
		if (id->ti_type & NRS_TBF_FLAG_GID)
...
static void unpack_ugid_from_mdt_rec(struct ptlrpc_request *req,
				     struct tbf_id *id)
{
			      ...
			      struct tbf_id *id)
{
	...
	case MDS_CLOSE:
	case MDS_REINT:
		unpack_ugid_from_mdt_rec(req, id);
		break;
	case LDLM_ENQUEUE: {
		struct ldlm_intent *lit;
		lit = req_capsule_client_get(&req->rq_pill, &RMF_LDLM_INTENT);
		 /* The following includes all the intent operations,
		    see function mdc_enqueue_base() for details */
		if (lit == NULL)
			/* case LDLM_FLOCK */
			rc = -EINVAL;
		else if (lit->opc & IT_GETXATTR || lit->opc & IT_GETXATTR ||
			 lit->opc & (IT_GETATTR | IT_LOOKUP))
			unpack_ugid_from_mdt_body(req, id);
		else if (lit->opc & IT_OPEN || lit->opc & IT_UNLINK)
			unpack_ugid_from_mdt_rec(req, id);
		else
			/* case IT_LAYOUT */
			rc = -EINVAL;
	...
	default:
		/* skip MDS_GET_INFO and MDS_STATFS */
		rc = -EINVAL;
		break;
	}
	return rc;
...
static int nrs_tbf_id_cli_set(struct ptlrpc_request *req,
			      struct tbf_id *id, __u32 ti_type)
{
	__u32 opc = lustre_msg_get_opc(req->rq_reqmsg);
	int rc = -EINVAL;
	...
	else if (opc > OST_LAST_OPC && opc < MDS_LAST_OPC)
		rc = mdt_tbf_id_cli_set(req, id);
	else if (opc == LDLM_ENQUEUE){
		__u32 req_portal = req->rq_import->imp_client->cli_request_portal;

		/* TODO: handle DATA on MDT case */
		if (req_portal == MDS_REQUEST_PORTAL)
			rc = mdt_tbf_id_cli_set(req, id);
		else if (req_portal == OST_REQUEST_PORTAL ||
				req_portal == OST_IO_PORTAL)
			rc = ost_tbf_id_cli_set(req, id);
	}
				       ...
				       char *keystr, int max_len)
{
	...
	snprintf(keystr, max_len, "%s_%s_%d_%u_%u", jobid,
		 libcfs_nid2str(req->rq_peer.nid), opc, id.ti_uid,
		 id.ti_gid);
	LASSERT(strlen(keystr) < max_len);
...
nrs_tbf_id_list_free(struct list_head *uid_list)
{
	...
	list_for_each_entry_safe(nti_id, n, uid_list, nti_linkage) {
		...
		OBD_FREE(nti_id, sizeof(struct nrs_tbf_id));
	}
}
...
static void *nrs_tbf_id_hop_object(struct hlist_node *hnode)
{
	return hlist_entry(hnode, struct nrs_tbf_client, tc_hnode);
}
...
static struct cfs_hash_ops nrs_tbf_id_hash_ops = {
	...
	.hs_object	= nrs_tbf_id_hop_object,
...
static int
nrs_tbf_id_list_add(const struct tbf_id id, struct list_head *id_list)
{
	struct nrs_tbf_id *nti_id;

	OBD_ALLOC(nti_id, sizeof(struct nrs_tbf_id));
	if (nti_id == NULL)
		return -ENOMEM;
...
{
	if (!list_empty(&cmd->u.tc_start.ts_ids))
		      ...
		      __u32 tif)
{
	...
	while (src.ls_str) {
		...
		rc = nrs_tbf_id_list_add(id, id_list);
		if (rc)
			break;
	}
out:
...
static int nrs_tbf_ug_id_parse(struct nrs_tbf_cmd *cmd, char *id)
{
	...
	memcpy(cmd->u.tc_start.ts_ids_str, src.ls_str, src.ls_len);
		     ...
		     struct nrs_tbf_cmd *start)
{
	...
	int rc = 0;
	__u32 tif = head->th_type_flag;

	LASSERT(start->u.tc_start.ts_ids_str);
	OBD_ALLOC(rule->tr_ids_str,
		  strlen(start->u.tc_start.ts_ids_str) + 1);
	if (rule->tr_ids_str == NULL)
		return -ENOMEM;

	memcpy(rule->tr_ids_str,
	       start->u.tc_start.ts_ids_str,
	       strlen(start->u.tc_start.ts_ids_str));
...
{
	if (!list_empty(&rule->tr_ids))
		nrs_tbf_id_list_free(&rule->tr_ids);
	LASSERT(rule->tr_ids_str != NULL);
...
static void nrs_tbf_cmd_fini(struct nrs_tbf_cmd *cmd)
{
	if (cmd->tc_cmd == NRS_CTL_TBF_START_RULE) {
		...
		case NRS_TBF_FLAG_GID:
			...
			break;
		default:
			CWARN("unkown NRS_TBF_FLAGS:0x%x\n",
